...
async def process_queue():
    ...
    while True:
        ...
        if is_processing:
            print("[BUSY] AI уже занят, отправляем заглушку только пользователю")
            try:
                await websocket.send_text(BUSY_MESSAGE)  # 🔥 Отправляем ТОЛЬКО пользователю, не всем
...
async def proxy_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # ✅ Если AI занят, отправляем заглушку ТОЛЬКО пользователю
            if is_processing:
                print("[BUSY] AI уже занят, мгновенно отправляем заглушку только клиенту")
                await websocket.send_text(BUSY_MESSAGE)  # 🔥 Только отправителю
